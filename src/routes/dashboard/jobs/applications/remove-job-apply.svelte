<script lang="ts">
	...
	import { removeJobApplicationSchema } from './apply-schema';

	let props: {
		item: Selectable<JobApplication>;
		form: SuperValidated<Infer<typeof removeJobApplicationSchema>>;
	} = $props();
	...
	const form = superForm(props.form, {
		validators: zod4Client(removeJobApplicationSchema),
		warnings: { duplicateId: false },
		resetForm: false,
		clearOnSubmit: 'none',
		onUpdate: ({ form: f }) => {
			if (f.valid) {
				toast.success(`Successfully removed job application!`, {
					id: process
				});
...
<div>
	<Sheet>
		<Tooltip>
			...
			<TooltipContent>
				<p>Remove job application</p>
			</TooltipContent>
		</Tooltip>

		<SheetContent showCloseButton={false} side="bottom" class="pt-10 pb-20">
			<SheetHeader>
				<SheetTitle class="text-center text-xl">Remove Job Application</SheetTitle>
